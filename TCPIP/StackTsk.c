...
void StackTask(void)
{
    ...
    while(1)
    {
        ...
        //WRITEHEX( "Got Packet cFrameType ", cFrameType);
        switch(cFrameType)
        {
        case MAC_ARP:
            ARPProcess();
        ...
        case MAC_IP:
            if(!IPGetHeader(&tempLocalIP, &remoteNode, &cIPFrameType, &dataCount))
                break;
#if defined(DEBUG_IP_PACKETS)
            WRITEHEX(" IP Frame type ", cIPFrameType);
            WRITEIP(" Target IP :", tempLocalIP.Val);
            WRITEIP(" RemoteNode : ", remoteNode.IPAddr.Val);
            WRITEHEX(" datacount :", dataCount);
            WRITEIP("My IP : ", AppConfig.MyIPAddr.Val );
#endif
