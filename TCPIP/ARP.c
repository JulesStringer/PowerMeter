...
#endif
    WRITEIPMAC(DEBUG_UDP, "ARP Reply to: ", packet->TargetIPAddr.Val, packet->TargetMACAddr.v);
    WRITEIPMAC(DEBUG_UDP, "ARP Sender  : ", packet->SenderIPAddr.Val, packet->SenderMACAddr.v);
    WRITESTRING(DEBUG_UDP, "\n");
...
BOOL ARPProcess(void)
{
    ...
    case SM_ARP_IDLE:
        ...
        MACDiscardRx();
        SwapARPPacket(&packet);
//    WRITEIPMAC("SM_ARP_IDLE Target: ", packet.TargetIPAddr.Val, packet.TargetMACAddr.v);
//    WRITEIPMAC("SM_ARP_IDLE Sender: ", packet.SenderIPAddr.Val, packet.SenderMACAddr.v);
...
/*         #if defined(STACK_USE_AUTO_IP)
        ...
        if(packet.Operation == ARP_OPERATION_REQ)
        {
            ...
            //WRITEIPMAC("ARP Sender ", packet.SenderIPAddr.Val, packet.SenderMACAddr.v);
            //WRITEIPMAC("ARP Target ", packet.TargetIPAddr.Val, packet.TargetMACAddr.v);
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
...
#define KS_ARP_IP_MULTICAST_HACK y
	...
	packet.TargetIPAddr
                = ((AppConfig.MyIPAddr.Val ^ IPAddr->Val) & AppConfig.MyMask.Val) ? AppConfig.MyGateway : *IPAddr;
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
	packet.SenderIPAddr			= AppConfig.MyIPAddr;
#endif
...
void ARPAnnounce()
{
    ...
    packet.Operation = ARP_OPERATION_REQ;
    packet.HardwareType = HW_ETHERNET;
    packet.Protocol = sizeof(MAC_ADDR);
    packet.ProtocolLen = sizeof(IP_ADDR);
